...
class Basic_blocks(nn.Module):
    def __init__(self, in_channel, out_channel, q=3):
        ...
        self.conv=SelfONNLayer(in_channel,out_channel,1)
        self.conv1 = nn.Sequential(
            SelfONNLayer(out_channel, out_channel, 3, padding=1),
...
class DSE(nn.Module):
    def __init__(self, in_channel, decay=2):
        ...
        self.layer1 = nn.Sequential(
            SelfONNLayer(in_channel, in_channel // decay, 1),
            nn.Tanh(),
            SelfONNLayer(in_channel // decay, in_channel, 1),
            nn.Sigmoid()
        )
...
class Spaceatt(nn.Module):
    def __init__(self, in_channel, decay=2):
        ...
        self.Q = nn.Sequential(
            ...
            SelfONNLayer(in_channel // decay, 1, 1),
            nn.Sigmoid()
        )
        self.K = nn.Sequential(
            SelfONNLayer(in_channel, in_channel // decay, 3, padding=1),
            nn.BatchNorm2d(in_channel//decay),
            SelfONNLayer(in_channel//decay, in_channel//decay, 3, padding=1),
        ...
        self.sig = nn.Sequential(
            SelfONNLayer(in_channel // decay, in_channel, 3, padding=1),
...
class CSCA_blocks(nn.Module):
    def __init__(self, in_channel, out_channel, decay=2, q=3):
        super(CSCA_blocks, self).__init__()
        self.upsample = nn.ConvTranspose2d(
            in_channel, in_channel, 2, stride=2)
        self.conv = Basic_blocks(in_channel*2, out_channel // 2, q=q)
...
class FullSelfONN_CSCAUNet_DenseNet(nn.Module):
    def __init__(self, in_channels, n_class=1, decay=2, q=3):
        super(FullSelfONN_CSCAUNet_DenseNet, self).__init__()
        ...
        self.densenet.features.conv0 = SelfONNLayer(in_channels, 64, kernel_size=7, stride=2, padding=3, bias=False)

        self.center_dse = DSE(1024, decay)
        
        # Decoder
        self.up_conv5 = CSCA_blocks(1024, 512, decay, q=q)
        self.up_conv4 = CSCA_blocks(512, 256, decay, q=q)
        self.up_conv3 = CSCA_blocks(256, 128, decay, q=q)
        self.up_conv2 = CSCA_blocks(128, 64, decay, q=q)
        self.up_conv1 = CSCA_blocks(64, 32, decay, q=q)

        self.skip_covn5 = SelfONNLayer(1024, 512, 1)
        self.skip_covn4 = SelfONNLayer(512, 256, 1)
        self.skip_covn3 = SelfONNLayer(256, 128, 1)
        # self.skip_covn2 = SelfONNLayer(128, 64, 1)

        self.dp6 = SelfONNLayer(1024, n_class, 1)
        self.dp5 = SelfONNLayer(1024, n_class, 1)
        self.dp4 = SelfONNLayer(512, n_class, 1)
        self.dp3 = SelfONNLayer(256, n_class, 1)
        self.dp2 = SelfONNLayer(128, n_class, 1)
        self.out = SelfONNLayer(32, n_class, 3, padding=1)

        self.center5 = SelfONNLayer(1024, 512, 1, q=q)
        self.decodeup4 = SelfONNLayer(512, 256, 1, q=q)
        self.decodeup3 = SelfONNLayer(256, 128, 1, q=q)
        self.decodeup2 = SelfONNLayer(128, 64, 1, q=q)
